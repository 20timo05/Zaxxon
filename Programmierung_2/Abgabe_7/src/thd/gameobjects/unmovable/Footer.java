...
public class Footer extends GameObject {
    ...
    public Footer(GameView gameView, GamePlayManager gamePlayManager) {
        ...
        flagDisplayWidth = FooterBlockImages.FLAG.split("\n")[0].length() * FLAG_DISPLAY_SIZE;
        liveDisplayWidth = FooterBlockImages.LIVE.split("\n")[FooterBlockImages.LIVE.split("\n").length - 1].length() * LIVE_DISPLAY_SIZE;
    ...
    public void addToCanvas() {
        ...
        for (int i = 0; i < gamePlayManager.getLives(); i++) {
            gameView.addBlockImageToCanvas(FooterBlockImages.LIVE, 50 + liveDisplayWidth*1.5*i, GameSettings.GAME_HEIGHT + GameSettings.FOOTER_HEIGHT * 0.2, LIVE_DISPLAY_SIZE, 0);
        }
