...
public class WallRow extends CollidingGameObject {
    ...
    public WallRow(
        ...
        size = Math.floor(GameSettings.MAX_PLAYER_ALTITUDE / (10 * height));

        double offsetY = altitudeIndex * WallBlockGraphicUtils.FULL_BLOCK_INCREASE_OFFSET_Y * size;
        position.up(offsetY);
    ...
    private Polygon[] calculateHitbox(ArrayList<int[]> hitboxIndices) {
        ...
        for (int i = 0; i < hitboxIndices.size(); i++) {
            ...
            Position[] preProjectionRelativeHitbox = new Position[] {
                    new Position(idx[0]*size*WallBlockGraphicUtils.HALF_BLOCK_INCREASE_OFFSET_X, 0),
                    new Position((idx[1]+1)*size*WallBlockGraphicUtils.HALF_BLOCK_INCREASE_OFFSET_X, 0),
                    new Position((idx[1]+1)*size*WallBlockGraphicUtils.HALF_BLOCK_INCREASE_OFFSET_X, -1*size*WallBlockGraphicUtils.FULL_BLOCK_INCREASE_OFFSET_Y),
                    new Position(idx[0]*size*WallBlockGraphicUtils.HALF_BLOCK_INCREASE_OFFSET_X, -1*size*WallBlockGraphicUtils.FULL_BLOCK_INCREASE_OFFSET_Y)
            };
