...
/**
 * The {@code EnergyBarrierAnimation} is only the yellow Grid, indicating the position.
 * The actual Barrier is the corresponding Parten {@link EnergyBarrier}, which actually is a {@link thd.gameobjects.base.CollidingGameObject}.
 *
 * @see GameObject
 * @see EnergyBarrier
 */
class EnergyBarrierAnimation extends GameObject {
     ...
     * @param dimensions      an array with height, width and size of the BlockGraphic
     */
    EnergyBarrierAnimation(GameView gameView, GamePlayManager gamePlayManager, double distanceFromSpawnLine, int altitudeLevel, double[] dimensions) {
        super(gameView, gamePlayManager, distanceFromSpawnLine, 0);
        ...
        offsetTower = new Vector2d(TRAVEL_PATH_CALCULATOR.getTravelPathWidth() * movementInterpolation, -GameSettings.MOVEMENT_ANGLE_IN_RADIANS);
    }

    /**
     * Moves {@code EnergyBarrier} object forward, i.e. relative to the corresponding {@link EnergyBarrier}.
     *
     * @param towerPosition the current position of the corresponding {@link EnergyBarrier}
     * @see EnergyBarrier
     */
